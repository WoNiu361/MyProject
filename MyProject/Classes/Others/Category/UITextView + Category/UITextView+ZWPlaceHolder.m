...
- (void)setTextChanged:(void (^)(NSString *text))textChanged{
   objc_setAssociatedObject(self, @selector(textChanged), textChanged, OBJC_ASSOCIATION_COPY_NONATOMIC);
}

- (void (^)(NSString *text))textChanged{
     return objc_getAssociatedObject(self, @selector(textChanged));
}
...
-(UILabel *)zw_placeHolderLabel{
    ...
    if (!placeHolderLab) {
        ...
        placeHolderLab.textColor = [[UIColor lightGrayColor] colorWithAlphaComponent:0.8];
